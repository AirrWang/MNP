 ...
 */
public class ListAdapterExerOptions extends BaseAdapter {
    ...
    private TitleRecordPracticeCompetenceDao dao;
    private TitleRecordPracticeCompetence titleRecordPracticeCompetence;

    public ListAdapterExerOptions(Context context, int position, View view, ViewPager vp) {
        this.context = context;
        this.pos = position;
        this.view = view;
        this.vp = vp;
        dao = DbCore.getDaoSession().getTitleRecordPracticeCompetenceDao();
    }
    ...
    private void initSaveRightNum(int pos) {
        if (titleRecordPracticeCompetence == null) {
            titleRecordPracticeCompetence = dao.queryBuilder()
                    .where(TitleRecordPracticeCompetenceDao.Properties.Q_id.eq(dataSource.get(pos).getQ_id()))
                    .build().unique();
        } else {
            s[0] = titleRecordPracticeCompetence.getS1();
            s[1] = titleRecordPracticeCompetence.getS2();
            s[2] = titleRecordPracticeCompetence.getS3();
            s[3] = titleRecordPracticeCompetence.getS4();
            s[4] = titleRecordPracticeCompetence.getS5();
        }
    }
    ...
    private void initErr(String type) {
        if (titleRecordPracticeCompetence == null) {
            titleRecordPracticeCompetence = new TitleRecordPracticeCompetence(dataSource.get(pos).getC_id(), dataSource.get(pos).getQ_id(),
                    s[0], s[1], s[2], s[3], s[4], type);
            dao.insertOrReplaceInTx(titleRecordPracticeCompetence);
            dao.detachAll();
        } else {
            titleRecordPracticeCompetence.setS1(s[0]);
            titleRecordPracticeCompetence.setS1(s[1]);
            titleRecordPracticeCompetence.setS1(s[2]);
            titleRecordPracticeCompetence.setS1(s[3]);
            titleRecordPracticeCompetence.setS1(s[4]);
            titleRecordPracticeCompetence.setIs_right(type);
        }
