...
/**
 * Created by NiPing and Airr Wang on 2017/7/11.
...
public class PwdCheckUtil {
    ...
    public static void whatIsInput(Context context, EditText edInput) {
        ...
        if (m.matches()) {
            ToastUtils.showShort(context, "输入的是数字");
        }
        p = Pattern.compile("[a-zA-Z]");
        m = p.matcher(txt);
        if (m.matches()) {
            ToastUtils.showShort(context, "输入的是字母");
        }
        p = Pattern.compile("[\u4e00-\u9fa5]");
        m = p.matcher(txt);
        if (m.matches()) {
            ToastUtils.showShort(context, "输入的是汉字");
        }
    }

    /**
     * 判断EditText输入的数字、中文还是字母方法,用于判断tel值是否为数字来进行准确认证
     */
    public static boolean whatIsInputTel(Context context, String tel) {
